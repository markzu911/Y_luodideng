...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (params.viewType === "far") {
          preservationGuidance = "2. UNIFIED AND COHESIVE ROOM DESIGN (PRESERVE STYLE AND ESSENCE / 保持风格与美感统一): The generated room MUST maintain the exact interior style, furniture designs, color palette, wall textures, window curtains, and cozy atmosphere of the Reference Room Image. However, the camera shooting angle, perspective, or position is highly flexible. It does NOT have to be identical to the uploaded room shooting perspective. You can shift, rotate, or elevate the camera angle to better integrate and showcase the floor lamp, as long as the room feels like the same environment. 必须完美保持参考房间的软装硬装风格、配色、墙面材质、窗帘质感和温馨氛围。但是，相机的拍摄角度、视点和高度是完全自由且可以修改的，绝对不强制要求和用户上传的房间拍摄角度一模一样！可以进行旋转、平移或偏移视角，以便更加优美、更自然地融入并展现落地灯。";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (FAR VIEW / 全景/远景): This is a wide-angle spacious full-shot (远景) displaying the layout of the room. The camera is placed back to show the main furniture (e.g., the full bed or the main sofa set) with the floor lamp integrated as a small-to-medium element. Feel free to adjust the camera perspective or viewing angle relative to the reference image to position the lamp elegantly and show it off in the room's wider context.";
        } else if (params.viewType === "mid") {
          preservationGuidance = "2. UNIFIED AND COHESIVE ROOM DESIGN (PRESERVE STYLE AND ESSENCE IN COZY MID-SHOT / 中景保持风格与美感统一): The generated image is a CLOSE-UP MID SHOT (中景/中近景). It MUST crop the room around the head of the bed or side of the sofa where the lamp is placed. The camera position, angle, and distance can change freely compared to the reference image to achieve the most beautiful, balanced composition. Do NOT force the same camera angle. You can shoot from a cozy side angle, a front-facing perspective, or a 45-degree angle. The style, materials, fabrics, and textures of the visible furniture (e.g., the bed's headboard, the pillow, the sofa fabric) must perfectly match the Reference Room Image. 这是一个【中景/中近景】构图，必须进行局部的裁剪和放大，聚焦于落地灯及其相邻的家具。不需要与参考图的拍摄角度保持一致，视角可自由偏转、旋转或调整，以便能更好地展现落地灯及光影细节。剪裁面内的家具细节（如床头板、枕头、沙发和抱枕材质）必须与参考房间的高级质感完全一致！";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MID VIEW / 中景/中近景): This is a classic medium-distance cinematic shot (中景) taken from 1.5 to 2.5 meters away, focusing on the floor lamp standing elegantly as the vertical focal point (occupying 65-75% of the frame height). Only a partial section of the sofa or bed is shown. The camera angle should be adjusted to show the lamp and its warm ambient glow projecting onto surrounding furniture in the most aesthetically pleasing composition. Crop out the ceiling and far corners of the room.";
        } else if (params.viewType === "close") {
          preservationGuidance = "2. UNIFIED AND COHESIVE ROOM DESIGN (PRESERVE MATERIAL DETAILS IN EXTREMELY TIGHT CLOSE SHOT / 近景特写保持风格与细节统一): The generated image is an EXTREMELY TIGHT CLOSE-UP (特写/近景). The camera focuses closely on the floor lamp body and lampshade. You have absolute freedom in selecting the camera angle (e.g., shooting slightly upwards, downwards, or from a close-up side angle) to show off the lamp's intricate textures, finish, and delicate bulb glow. Keep the surrounding textures (like a bedside table top or sofa fabric texture) completely aligned in material and color with the Reference Room Image. 这是一个【特写/近景】构图，相机极近距离对准落地灯本身。拍摄角度可任意调整（如微仰拍、侧向特写等），只要能够最精细、最唯美地表现落地灯的材质、工艺、造型和温润灯光即可。";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (CLOSE VIEW / 近景/特写): This is a tight close-up (特写) focusing heavily and strictly on the floor lamp body and lampshade. Show the lamp itself and the immediate surface next to it (e.g., the bedside table, the headboard, or the sofa fabric) matching the Reference Room Image's design, style, materials, and structure. Optimize the camera angle to highlight the delicate lampshade pleats, metal poles, or table textures, and the beautiful local light pool.";
        }
