...
You must return a highly detailed analysis in a clean JSON format matching this exact schema:
{
  ...
  "layout": "Room layout description in Chinese highlighting space, key structural elements, and natural lighting",
...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (safeParams.viewType === "far") {
          preservationGuidance = "2. CENTERED LAMP IN LOCALIZED CORNER (远景视角 - 局部取景且落地灯居中):\n   - MAIN SUBJECT & CENTERING: The floor lamp MUST be the absolute main subject and MUST be positioned perfectly in the CENTER of the image frame. \n   - LOCALIZED CORNER BACKGROUND (严禁展示整个房间!): DO NOT render a wide full-room shot. The background must be a tightly framed localized corner of the authentic uploaded room (e.g., just one end of the sofa, a side table, and the wall). \n   - LOGICAL PLACEMENT FIRST: You must logically place the lamp beside the sofa armrest or corner FIRST, and THEN frame the camera so the lamp is perfectly centered while only capturing the adjacent furniture/wall as the background.\n   - FULL LAMP HEIGHT: Show the complete floor lamp from top to bottom base.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (FAR VIEW / 远景视角): The floor lamp is perfectly CENTERED in the image. The camera frames a localized section of the room (e.g., one side of the sofa) as the background. NEVER show the entire room.";
        } else if (safeParams.viewType === "close") {
          preservationGuidance = "2. AUTHENTIC MACRO PHOTOGRAPHY (真实近景特写摄影 - 完美融入真实房间一角且落地灯居中):\n   - SUBJECT FOCUS & CENTERING: The floor lamp (lampshade and upper pole) MUST be perfectly CENTERED and dominate the foreground.\n   - CROP LEVEL: You MUST crop out the lower half and base of the floor lamp.\n   - TRUE ENVIRONMENTAL INTEGRATION (拒绝抠图贴图感): The lamp MUST NOT look like it was simply enlarged and pasted over the room. It must be deeply integrated into the environment. The light from the lampshade must realistically spill over and illuminate the adjacent background elements.\n   - REALISTIC CORNER BACKGROUND (真实的房间一角背景): The background MUST be a realistic, localized corner of the room, such as a bedside table with a vase, part of a sofa, or a wall with a painting. STRICTLY FORBIDDEN to show the entire room layout or just a blurry blank wall. It must feel like a real corner context.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MACRO CLOSE-UP VIEW / 真实近景特写视角): A highly realistic, aesthetically pleasing macro shot. The camera is very close to the lampshade. The background showcases a beautifully framed real corner of the room (like a side table with flowers or edge of furniture) rather than the full room or a blank wall.";
        }
...
4. STRICT LAMP PLACEMENT RULES & REALISTIC SPATIAL PERSPECTIVE (落地灯摆放位置与透视空间关系):
   - ${safeParams.viewType === "close" ? "FOR CLOSE-UP VIEW (近景特写): The camera is extremely zoomed in on the lamp itself. The spatial relationship to the sofa/bed is implied by the background, but the lamp stands as the central focal point." : "CRITICAL SOFA PLACEMENT & PHYSICAL PROXIMITY: The floor lamp MUST be placed directly beside the sofa armrest or in the corner wall right behind the armrest. The lamp pole MUST be physically close to the sofa armrest with ZERO floating gap. STRICTLY FORBIDDEN: DO NOT place the floor lamp isolated in the middle of the room, in open pathways, or in front of the sofa! (绝对禁止将落地灯孤立放置在房间中央空地或离沙发太远的空旷处！绝对禁止把落地灯挡在沙发前面！必须紧贴沙发扶手或墙角放置！).\n   - CRITICAL BEDROOM PLACEMENT: In a bedroom, the floor lamp MUST be placed ONLY at the headboard corner beside the nightstand. STRICTLY FORBIDDEN to place at the foot of the bed or bed-end bench (绝对禁止把落地灯放在床尾或床脚处！).\n   - PERSPECTIVE ALIGNMENT: The sofa armrest, headboard, or side table MUST be directly adjacent to the lamp pole in the camera view to establish a natural, physically realistic spatial perspective."}

5. CAMERA CENTERING & VIEW-TYPE PERSPECTIVE (相机镜头对焦取景与自由机位):
   - FREE & OPTIMIZED CAMERA ANGLE (自由最佳机位): You DO NOT need to strictly copy the original camera shooting angle from the uploaded room image. You are FREE to choose the BEST, most aesthetically pleasing, and professional photography camera angle/position to showcase the floor lamp and the adjacent furniture perfectly.
   - ${perspectiveGuidance}
