...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (params.viewType === "far") {
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (FAR VIEW / 全景/远景): This MUST be a very wide-angle, spacious full-shot (远景) displaying the complete room layout. The camera MUST be placed very far back to show the entire furniture arrangement, the full length of the bed (from headboard to footboard), the entire long sofa from end to end, the full walkway floors, ceiling, windows, and surrounding space. The floor lamp should be a small-to-medium element integrated in the wide room background. Keep all background elements, walls, windows, and layout 100% identical to the Reference Room Image.";
        } else if (params.viewType === "mid") {
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MID VIEW / 中景/中近景): This MUST be a classic medium-distance cinematic shot (中景) taken from 2 to 2.5 meters away, exactly matching the reference composition. The camera is placed at eye-level or slightly above, showing only a partial section (about 50-60%) of the primary sofa or bed. The floor lamp stands elegantly as the vertical focal point, occupying 65-75% of the frame height. The foreground features the detailed textures of the sofa/bed and cozy throw pillows with warm light spilling beautifully over them. The background shows soft vertical curtain folds or wall panels, with side furniture partially cropped at the frame boundaries. DO NOT show the entire room, do NOT show the ceiling or entire floor, and do NOT show the full sofa/bed from end to end. Keep the layout and structure completely faithful to the reference room but zoomed in to this exact cozy mid-distance.";
        } else if (params.viewType === "close") {
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (CLOSE VIEW / 近景/特写): This MUST be a tight close-up (特写) focusing heavily and strictly on the floor lamp body and lampshade. Show the lamp itself and the immediate surface next to it (e.g., the bedside table, the headboard, or the sofa fabric) exactly matching the Reference Room Image's design, style, materials, and structure without any alteration. Keep the background layout and structure 100% identical to the reference room, just zoomed in extremely close to the lamp to show the delicate materials, finish, and the glow. DO NOT invent new furniture shapes or change the background walls.";
        }
