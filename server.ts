...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (params.viewType === "far") {
          ...
          preservationGuidance = "2. EXTREME CLOSE-UP DETAIL SHOT (近景/灯罩特写): Macro-level product detail view. The camera MUST zoom in very closely to focus exclusively on the upper lampshade and the straight vertical upper metal pole. The lampshade MUST dominate 60%-70% of the entire image width and height. CRITICAL: The floor, lamp base, room ceiling, and entire furniture bodies MUST be completely cropped OUT of the frame. Ensure the upper straight pole is perfectly clean and elegant. On the lower-left or lower-right corner, a tiny portion of a nearby surface (like a bedside table or console) containing a delicate ceramic flower vase with blooming roses (such as pink roses) can be visible in soft focus to add a cozy, authentic lifestyle touch, mirroring the provided reference image composition exactly.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (CLOSE VIEW / 近景/灯罩特写): Macro photography focus directly on the lampshade and straight vertical pole. The lampshade occupies the main center-top of the frame. Only the upper shade and straight vertical pole are visible. Place a partial table-top detail (e.g., corner of a nightstand with a delicate vase and flower) closely beside the pole to anchor the setting, keeping the background extremely simple (e.g., plain paneled wall or closet door). DO NOT add any horizontal swing arm, side bracket, or swivel joint unless it exists on the original uploaded lamp!";
        }
