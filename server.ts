...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (safeParams.viewType === "far") {
          ...
          preservationGuidance = "2. CLOSE-UP MACRO DETAIL SHOT (近景特写视角 - 局部实拍感):\n   - SUBJECT FOCUS: The floor lamp (lampshade and upper pole) is the ABSOLUTE MAIN SUBJECT. The camera must be zoomed in extremely close to the lamp.\n   - CROP LEVEL: You MUST crop out the lower half and base of the floor lamp. The lampshade MUST dominate the central frame.\n   - NARROW FIELD OF VIEW BACKGROUND (背景必须是局部): Because the camera is very close, the background MUST be a NARROW, PARTIAL section of the room (e.g., just a piece of the textured wall, a curtain edge, or top of a cabinet/sofa). STRICTLY FORBIDDEN to show the wide layout of the entire room in the background! It must look like a real, natural close-up photograph taken inside the room, not a lamp pasted over a wide room picture.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MACRO CLOSE-UP VIEW / 近景特写视角): Camera zoomed in very close to the lampshade. The background field of view is extremely narrow, showing only a small, localized fraction of the room directly behind the lamp.";
        }
