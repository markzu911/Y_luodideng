...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (params.viewType === "far") {
          ...
          preservationGuidance = "2. CLOSE-UP SHOT DESIGN & PERSPECTIVE (近景/特写空间构图): Premium indoor furniture photography, shot like a high-end home decoration magazine (室内摄影，像高端家居杂志拍摄). Use a horizontal eye-level camera angle, positioned slightly close to the main subject for a tight, intimate close-up shot (平视机位，略微靠近主体，近距离取景，局部特写式空间构图，画面横向构图). Frame the scene using a sophisticated rule-of-thirds spatial layout where the primary focus is directly on the floor lamp (三分构图，主体聚焦在落地灯). The lampshade occupies the main focal sweet-spot (around 60%-70% of the visual space). Crucially, the physical pole, neck, support arm, or arched bracket of the floor lamp MUST strictly match and preserve the exact shape, metal texture, color, and curvature of the user's uploaded reference image—whether it is a straight vertical rod, an elegant sweeping arch, or a curved neck (灯罩下方延伸出的灯杆/支架结构，必须高度还原原图中对应的物理形状、金属色调与弯曲度/拱形特征，不可一概而论). As foreground, elegantly frame a cozy partial slice of a sofa (前景为沙发), while the background presents a layered depth of field with subtle wall paneling or furniture elements in soft blur (背景有层次感，镜头焦距约 50mm，浅景深，自然透视，视角真实). This keeps the scene incredibly cozy, immersive, and professionally composed.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (CLOSE VIEW / 近景特写视角): An elegant close-up, horizontal composition at a frontal eye-level view (平视机位). The camera is positioned at a natural, slightly close distance with a 50mm focal length to capture the floor lamp as the absolute hero subject (主体聚焦在落地灯). Incorporate a cozy partial portion of the sofa in the foreground (前景为沙发) and a beautifully layered background paneling (背景有层次感). Ensure a professional shallow depth of field (浅景深, 50mm lens) with a completely natural perspective (自然透视，视角真实). The structural design, metal colors, pole curvature, or arch of the lamp MUST strictly copy the user's uploaded reference lamp.";
        }
