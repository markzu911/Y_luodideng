...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (safeParams.viewType === "far") {
          ...
          preservationGuidance = "2. MEDIUM CLOSE-UP SHOT (中景/拉近视角 - 在远景基础上显著拉近镜头): Based on the Far View corner composition, ZOOM IN the camera significantly closer! Focus directly on the upper half/two-thirds of the floor lamp (lampshade, soft ambient light glow, upper pole/bend) and immediate nearby furniture (top of sofa backrest/armrest, side table with decor like flowers or books). The lampshade should occupy a prominent, central role in the upper frame while preserving the exact same room wall, curtains, and furniture background as the Far View.";
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MID VIEW / 中景/拉近镜头视角): CRITICAL MEDIUM CLOSE SHOT! Do NOT show a full-height shot down to the lamp base. Pull the camera closer in (像镜头拉近特写) to frame from mid-pole up to the lampshade, capturing the lampshade texture, warm light diffusion, and the top section of the sofa or side table. The bottom base of the lamp can be naturally cropped off at the bottom of the frame.";
