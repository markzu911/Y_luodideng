...
2. ROOM REGENERATION: Do NOT directly edit the original room photograph. You MUST reconstruct a new room based on the analysis results of the original room and place the floor lamp inside it. CRITICAL: DO NOT alter the architectural structure, the room layout, or the styles of the furniture. DO NOT add any fake walls, pillars, or partitions to block the view. DO NOT add any extra furniture (like extra sofas or chairs). The architectural structure, furniture count, furniture styles, and layout MUST remain exactly the same as the original room. 绝对不能改变房间的布局和家具的样式，不允许直接在原来的房间图片上进行修改，必须根据分析的房间数据重构一个房间，然后再把落地灯放进去！绝对不能在画面中生硬地添加一堵墙或柱子！
