...
Return only the raw JSON. Do not wrap it in markdown code blocks like \`\`\`json.`,
        ...
        if (params.viewType === "far") {
          ...
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (MID VIEW / 中景): MUST show a medium shot perspective. The camera is noticeably closer than the far view. It focuses on the local zone where the lamp is placed. CRITICAL: The background MUST remain 100% IDENTICAL to the original image, just zoomed in. Do NOT change the camera angle or perspective from the original photo. DO NOT invent or add any new furniture, decorations, or objects that are not in the original room. 必须保持和原图完全一致的背景布局和家具，不要改变相机视角，仅仅是拉近镜头（Zoom in）。绝对不能凭空捏造原图中没有的物品！";
        } else if (params.viewType === "close") {
          perspectiveGuidance = "4. VIEW AND PERSPECTIVE (CLOSE VIEW / 近景/特写): MUST show a tight close-up perspective. The camera is very close, focusing primarily on the lampshade and the light it emits. CRITICAL: The background MUST remain 100% IDENTICAL to the original image, just tightly zoomed in. Do NOT change the camera angle or perspective from the original photo. ABSOLUTELY NO new objects, toys, plushies, partitions, or furniture can be added. 必须保持和原图完全一样的背景细节，仅仅是拉近镜头（Zoom in）特写灯罩。绝对不能改变原图视角，绝对、绝对不能凭空捏造原图中没有的家具或物品（例如多出来的柜子、玩偶、隔断等）！";
        }
